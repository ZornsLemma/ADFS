# This has been hacked to be very "generous" with its estimates of useful
# savings; it doesn't really have any logic behind it, but I was hoping it
# would point up cases where it would pay to merge an instruction into one of
# the 'chunk' subroutines even if it wouldn't pay to add a separate subroutine.
# This is a mis-use of this script but if you take the output with a small
# mountain of salt it can give useful hints.
import sys
...
def possible_saving(candidate_sequence):
    ...
    if 'JSR' not in candidate_string:
        return -1
    if ':' not in candidate_string:
        return -1
    saving = -candidate_bytes
